...
class ManageProjectViewModel extends BaseViewModel{
  ...
  pickProjectEndDate(BuildContext context) async {
    ...
    if (pickedDate != null) {
      ...
      _projectEndDate.text = "${pickedDate.day.toString().padLeft(2, '0')}-${pickedDate.month.toString().padLeft(2, '0')}-${pickedDate.year}";
    }
  }
   addProject() async{
    print("We will add a new project") ;
    if (formkey.currentState!.validate()) {
      updateState(LoadingState(stateRendererType: StateRendererType.fullScreenLoadingState));
      (await _manageProjectUseCase.addProject(Project.request(_projectName.text.trim(), _projectDescription.text.trim(), _projectEndDate.text.trim())))
          .fold((failure) {
  ...
  editProjectDetails() async {
    print("We will modify the project") ;
